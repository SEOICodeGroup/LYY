...
struct Code {
	...
	Code(int x,int e[][4], int belong, char f) {
		hash = x;
		if(e[1][1] < 1 && e[1][1] > 9) decoding();
		if(x == 0) coding();
		belong = belong;
		f = f;
	}
...
void inh(Code x, char f, int v) {
        ...
        if(v == 1) q1.push(x.hash);
        if(v == 2) q2.push(x.hash);
        h[t].push_back(x);
}
